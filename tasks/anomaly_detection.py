...
def eval_ad_result(test_pred_list, test_labels_list, delay):
    ...
    return {
        'f1': ' '+str(f1_score(labels, pred))+' ',
        'precision': ' '+str(precision_score(labels, pred))+' ',
        'recall': ' '+str(recall_score(labels, pred))+' ',
        'acc ': ' '+str(accuracy_score(labels, pred))+' ',
        'roc_auc':' '+str(roc_auc_score(labels, pred))+' ',
        # 'auc': auc(labels, pred),
    }
